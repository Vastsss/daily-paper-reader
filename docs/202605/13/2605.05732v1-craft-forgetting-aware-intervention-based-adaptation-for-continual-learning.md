...
method: 通过在隐藏表示空间学习低秩干预，并利用KL散度统一任务路由、正则化微调和干预合并过程。
result: 在多个基准和模型规模上，CRAFT相比LoRA等强基准方法显著提升了整体性能并减少了遗忘，且对任务顺序具有鲁棒性。
conclusion: 在表示空间中通过输出空间散度引导的干预控制，为大语言模型的持续学习提供了一种可扩展且原则性的方法。
---
