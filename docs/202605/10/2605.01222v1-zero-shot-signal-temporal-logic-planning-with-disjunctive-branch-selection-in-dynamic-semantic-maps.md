...
tags: ["query:ts-fm"]
score: 6.0
evidence: 使用Transformer进行零样本信号时序逻辑规划
...
method: 提出一种集成地图条件Transformer与启发式分支选择的求解器，并利用传递强化学习实现任务分解与逻辑一致性。
result: 在具有多样障碍物布局的动态语义地图实验中，该方法在零样本泛化和STL任务覆盖率上表现优异。
conclusion: 该框架为动态环境下的复杂逻辑任务规划提供了一种高效且无需重新训练的零样本解决方案。
---

## 摘要
信号时序逻辑 (STL) 提供了可验证的任务规范，对于安全关键型控制至关重要。然而，STL 规划仍面临挑战：基于精确优化的方法通常速度较慢，而基于学习的方法难以在不同环境间泛化。我们提出了一种适用于可变地图环境的零样本 STL 规划求解器，无需重新训练即可生成可行轨迹。通过将地图条件的 Transformer 架构与轻量级启发式算法相结合，我们的方法能够有效处理复杂的析取 (OR) 子公式。此外，我们利用传递强化学习 (TRL) 来确保分解后的子任务在时间接地和逻辑连贯性上保持一致。在具有多种障碍物布局的动态语义地图上进行的实验表明，该框架取得了持续的性能提升，突显了其在多变环境下的卓越零样本泛化能力以及广泛的 STL 覆盖范围。
