...
title_zh: 防患于未然：多智能体系统中基于前瞻引导的防御
...
conclusion: 本研究证明了通过预见性引导的局部净化策略可以有效抵御多智能体系统中的传染性攻击，同时维持系统的协作多样性。
---

## 摘要
基于大模型的多智能体系统（MASs）通过专业化智能体实现协作解决复杂问题。然而，MASs 容易受到传染性越狱（infectious jailbreak）的影响，即攻击单个智能体可能会扩散到其他智能体，导致大规模沦陷。现有的防御措施通过训练一种更具传染性的“治愈因子”来应对，使智能体倾向于检索该因子而非病毒对抗样本（VirAEs）。然而，这会导致智能体响应的同质化，仅提供表面的抑制而非真正的恢复。我们重新审视了这些防御措施，它们通过共享的治愈因子进行全局操作，而传染性越狱则源于局部交互行为。这种不匹配限制了它们的有效性。为解决这一问题，我们提出了一种无需训练的前瞻引导局部净化（FLP）框架，其中每个智能体通过对未来交互进行推理来追踪行为演变并消除感染。具体而言，每个智能体模拟后续对话轮次中的未来行为轨迹。为了反映 MASs 的多样性，我们引入了多角色模拟策略，以在不同交互语境下实现鲁棒预测。随后，我们利用响应多样性作为诊断信号，通过分析基于角色的预测在检索结果和语义层面的不一致性来检测感染。对于受感染的智能体，我们应用局部净化：通过立即回滚相册来缓解近期感染，而长期感染则使用递归二分诊断（RBD）处理，该方法递归地划分图像相册并应用相同的诊断策略来定位并消除 VirAEs。实验表明，FLP 将最大累积感染率从 95% 以上降低到 5.47% 以下。此外，检索和语义指标与良性基准紧密匹配，表明其有效保留了交互多样性。
