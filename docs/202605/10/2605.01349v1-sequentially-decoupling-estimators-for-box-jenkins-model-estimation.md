...
## 摘要
本文针对 Box-Jenkins (BJ) 模型提出了一种一致且渐近有效的估计方法，该方法适用于开环和闭环数据条件，可作为加权零空间拟合（weighted null-space fitting）方法的一种替代方案。该方法包含两个阶段：初始的顺序解耦 (SD) 估计器，以及随后的 Gauss-Newton (GN) 细化步骤。SD 估计器由三个顺序最小二乘 (LS) 估计器构建而成：(i) 估计高阶带外部输入的自回归 (ARX) 模型；(ii) 通过辅助输出误差 (OE) 模型估计 BJ 模型的动态模型；(iii) 使用另一个辅助 OE 模型估计 BJ 模型的噪声模型。我们在标准正则性条件下证明了 SD 估计器的一致性，这利用了 ARX 和 OE 模型底层 LS 估计器的一致性。此外，我们证明了在 ARX 模型阶数满足温和增长条件的前提下，从 SD 估计器开始的单步 GN 迭代所产生的估计器在渐近意义上等价于预测误差法。仿真研究证实了所提方法的理论特性。
