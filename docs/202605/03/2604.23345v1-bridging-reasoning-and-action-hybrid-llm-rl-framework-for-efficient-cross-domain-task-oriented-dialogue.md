...
title_zh: 连接推理与行动：用于高效跨域任务导向型对话的混合 LLM-RL 框架
...
score: 6.0
evidence: 混合LLM-RL框架
tldr: 针对跨域任务型对话中长程规划与约束推理的挑战，本文提出VLK-RL框架。该框架结合了大语言模型（LLM）的推理能力与强化学习（RL）的长程优化优势。通过双角色交叉验证机制过滤LLM生成的约束信息，并将其转化为结构化状态引导RL策略学习。实验证明，该方法在多领域任务中显著提升了对话系统的泛化性与鲁棒性。
selection_source: fresh_fetch
motivation: LLM在长程对话中表现不稳定，而RL难以直接从对话中提取约束，且两者简单结合易受LLM幻觉干扰。
method: 提出VLK-RL框架，通过LLM提取约束并利用双角色交叉验证确保一致性，最后将结构化约束融入RL的状态表示中。
result: 实验表明，VLK-RL在多个基准测试的长程任务上显著优于现有的单模型基线方法。
conclusion: 通过验证后的LLM知识增强强化学习，能有效提升跨域任务型对话系统的推理能力与长程规划稳定性。
---

## 摘要
跨域任务导向型对话需要在规划长程、多轮行动的同时，对隐式和显式的可行性约束进行推理。大语言模型（LLMs）能够推断此类约束，但在长程任务中不可靠；而强化学习（RL）能够优化长程行为，却无法从原始对话中恢复约束。因此，简单地将 LLM 与 RL 耦合是脆弱的：未经验证或非结构化的 LLM 输出可能会破坏状态表示并误导策略学习。基于此，我们提出了经过验证的 LLM 知识赋能的强化学习（VLK-RL），这是一种使 LLM 衍生的约束推理可用于 RL 的混合框架。VLK-RL 首先利用 LLM 诱导候选约束，然后通过双角色交叉检验程序对其进行验证，以抑制幻觉和跨轮次的不一致性。经过验证的约束被映射到与本体对齐的槽值表示中，从而为 RL 策略优化生成结构化的、约束感知的状态。在多个基准测试上的实验表明，VLK-RL 显著提高了泛化性和鲁棒性，在长程任务上优于强大的单模型基准。
