...
tldr: 针对高波动时间序列中极端事件难以建模的问题，本文提出DeepLévy框架。该方法利用Lévy稳定分布刻画重尾特性，通过最小化特征函数差异解决了概率密度函数难以解析计算的挑战。DeepLévy结合混合机制自适应学习多组分参数，在处理极端波动和尾部风险预测方面显著优于现有深度概率预测模型。
selection_source: fresh_fetch
motivation: 传统深度概率预测模型难以捕捉高波动时间序列中的重尾分布和极端事件，且Lévy分布的概率密度函数难以直接进行似然推断。
method: 提出一种基于特征函数匹配的神经网络框架，通过混合Lévy稳定分布并自适应学习上下文相关的权重与参数来进行多步不确定性建模。
result: 在真实和合成数据集上的实验表明，DeepLévy在尾部风险指标上优于现有最先进的深度概率预测方法，尤其是在极端波动环境下表现出色。
conclusion: DeepLévy为非高斯重尾时间序列的不确定性量化提供了一种有效且灵活的深度学习解决方案。
---
