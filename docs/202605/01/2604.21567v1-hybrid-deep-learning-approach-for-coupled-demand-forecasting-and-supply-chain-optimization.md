...
tldr: 针对纺织和个人防护装备等需求波动剧烈的行业，本文提出了一种混合AI框架HAF-DS。该框架将基于LSTM的需求预测模块与混合整数线性规划（MILP）优化层相结合，实现了预测与决策的深度耦合。实验证明，该方法在降低预测误差的同时，显著减少了库存成本和缺货率，提升了服务水平，为现代供应链管理提供了一种可扩展且高效的解决方案。
selection_source: fresh_fetch
motivation: 传统的预测与优化方法往往相互独立，难以应对纺织等行业中剧烈的需求波动和供应不确定性。
method: 提出HAF-DS框架，利用LSTM捕捉时间序列依赖性，并结合MILP层进行成本最优的补货与分配决策。
result: "预测误差MAE降低了14.7%，库存成本减少5.4%，缺货率降低27.5%，服务水平提升至97.8%。"
conclusion: 将预测性预测与指令性优化相结合，能显著提升供应链的准确性与运营效率。
---

## 摘要
在需求波动大且供应不确定的行业（如纺织品和个人防护装备 (PPE)）中，供应链的韧性和效率至关重要。传统的预测和优化方法通常孤立运行，限制了其在现实世界中的有效性。本文提出了一种用于供需预测与优化的混合人工智能框架 (HAF-DS)，该框架集成了一个基于长短期记忆网络 (LSTM) 的需求预测模块和一个混合整数线性规划 (MILP) 优化层。LSTM 捕捉需求的时间和上下文依赖关系，而优化层则制定具有成本效益的补货和分配决策。该框架通过基于嵌入的特征表示和循环神经网络架构，共同最小化预测误差和运营成本。在纺织品销售和供应链数据集上的实验表明，与统计和深度学习基准模型相比，该框架在性能上有显著提升。在组合数据集上，HAF-DS 将平均绝对误差 (MAE) 从 15.04 降低到 12.83 (14.7%)，均方根误差 (RMSE) 从 19.53 降低到 17.11 (12.4%)，平均绝对百分比误差 (MAPE) 从 9.5% 降低到 8.1%。此外，库存成本降低了 5.4%，缺货率降低了 27.5%，服务水平从 95.5% 提高到 97.8%。这些结果证实，将预测性预测与规范性优化相结合可以提高准确性和效率，为现代纺织品和 PPE 供应链提供了一种可扩展且适应性强的解决方案。
