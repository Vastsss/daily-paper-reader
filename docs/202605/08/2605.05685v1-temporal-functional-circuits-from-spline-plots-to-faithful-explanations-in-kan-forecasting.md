...
## 摘要
与 MLP 不同，Kolmogorov-Arnold 网络 (KAN) 在每个连接上都暴露了显式的可学习边函数，从而在时间序列预测中实现了机械性解释。本文介绍了时间功能电路 (Temporal Functional Circuits)，这是一个将 KAN 边函数从潜在的可视化转化为忠实的、基于时间的解释的框架。该框架基于门控残差 KAN，将预测分解为线性基准和稀疏激活的 KAN 修正项。该框架：(i) 通过输出感知归因将每条边映射到输入滞后项；(ii) 根据学习到的激活范围对边进行排序；(iii) 通过包括归零和样条移除在内的边级干预来验证忠实性。移除学习到的 B 样条组件而保留基础 SiLU 项会导致预测性能下降，这证明了样条形状本身具有超出基础激活函数的预测价值。在四个复杂度递增的合成机制上，随着信号复杂度的增加，学习到的门控会逐渐扩大开启程度。在机制切换信号上，门控 KAN 的均方误差 (MSE) 比纯线性模型低 59%。在八个基准测试中，门控架构与线性、注意力和 MLP 等替代方案相比具有竞争力，同时提供了基于 MLP 的修正项无法提供的可解释边函数。
