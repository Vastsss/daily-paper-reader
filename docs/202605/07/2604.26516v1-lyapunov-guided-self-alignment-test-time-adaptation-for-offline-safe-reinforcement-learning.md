...
tags: ["query:llm"]
score: 6.0
evidence: 基于Transformer的强化学习框架
tldr: 针对离线安全强化学习在部署时因训练与测试环境差异导致的不安全行为，本文提出SAS框架。该框架基于Transformer架构，通过李雅普诺夫引导的自我对齐机制，在测试阶段生成并筛选符合安全条件的想象轨迹作为上下文提示，实现无需重新训练的测试时自适应，在降低违规成本的同时提升了智能体的安全性与泛化能力。
selection_source: fresh_fetch
motivation: 离线强化学习智能体在实际部署时，常因训练数据与真实环境的分布差异而产生不安全行为。
method: 提出SAS框架，利用Transformer通过李雅普诺夫条件筛选想象轨迹作为上下文提示，实现测试时的自我对齐与行为修正。
result: 在Safety Gymnasium和MuJoCo基准测试中，SAS在保持或提高收益的同时，显著降低了成本和失败率。
conclusion: SAS证明了通过李雅普诺夫引导的想象和上下文学习，可以在不更新参数的情况下有效提升离线安全强化学习的鲁棒性。
---
