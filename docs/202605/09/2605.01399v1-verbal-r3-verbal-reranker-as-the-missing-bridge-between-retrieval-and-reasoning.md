...
score: 7.0
evidence: 大语言模型推理与检索
tldr: 针对传统RAG在整合检索信息与推理能力方面的不足，本文提出Verbal-R3框架。该框架引入“口头注释”（Verbal Annotations）作为桥梁，通过分析性叙述明确查询与上下文的逻辑联系。Verbal-R3由生成器和口头重排序器组成，结合相关性引导的测试时缩放技术，显著提升了LLM在复杂问答任务中的准确性和上下文关联度，实现了检索与推理的深度融合。
selection_source: fresh_fetch
motivation: 传统RAG直接注入原始文本往往导致检索信息与大模型推理能力的整合效果不佳，缺乏明确的逻辑衔接。
method: 提出Verbal-R3框架，利用口头重排序器生成相关性评分和口头注释，以引导生成器的迭代检索与推理过程。
result: Verbal-R3在多个复杂问答基准测试中取得了当前最先进（SOTA）的性能表现。
conclusion: 口头注释是连接检索与推理的关键桥梁，结合测试时计算优化能显著增强RAG系统的有效性。
---
