...
title_zh: CRAFT：面向持续学习的遗忘感知型干预式适配
authors: "Md Anwar Hossen, Fatema Siddika, Juan Pablo Munoz, Tanya Roosta, Ali Jannesari"
date: 2026-05-07
pdf: "https://arxiv.org/pdf/2605.05732v2"
tags: ["query:llm-ts"]
score: 6.0
evidence: 微调LLM以获取新能力并避免灾难性遗忘
tldr: 针对大语言模型在持续学习中面临的灾难性遗忘问题，本文提出了CRAFT框架。该方法不直接更新模型权重，而是通过在隐藏表示上学习低秩干预来实现。CRAFT通过基于输出分布散度的任务路由、基于KL散度的正则化微调以及干预合并三个阶段，统一了路由、正则化和合并过程。实验表明，CRAFT在多个基准测试中优于LoRA等方法，有效减少了遗忘并提升了模型性能。
selection_source: fresh_fetch
motivation: 解决大语言模型在持续微调过程中因权重更新导致的灾难性遗忘问题。
method: 通过学习隐藏表示的低秩干预，并利用KL散度统一任务路由、正则化微调和干预合并三个阶段。
result: 在多种模型规模和基准测试下，CRAFT在整体性能和抗遗忘能力上均优于强基线LoRA方法，且对任务顺序具有鲁棒性。
conclusion: 在表示空间中通过输出空间散度引导的干预控制，是实现大语言模型可扩展且原则性持续学习的有效途径。
---

## 摘要
大语言模型（LLMs）可以通过微调获得新能力，但持续适配往往会导致灾难性遗忘。我们提出了 CRAFT，这是一种持续学习框架，它通过学习隐藏表示上的低秩干预（low-rank interventions）来避免更新模型权重。CRAFT 分三个阶段进行：首先根据输出分布散度将每个任务路由到一组相似任务中；然后，利用针对该组先前状态的 Kullback-Leibler (KL) 散度对模型进行微调，这直接控制了遗忘并决定了收敛；最后，利用相同的 KL 信号将更新任务的干预合并到共享表示中。这一设计通过单一的基于 KL 的目标函数统一了路由、正则化和合并。在多个基准测试和模型规模上，与强大的基于 LoRA 的方法相比，CRAFT 提高了整体性能并减少了遗忘，同时对任务顺序保持鲁棒。这些结果表明，在输出空间散度的指导下，控制表示空间中的适配为 LLM 的持续学习提供了一种可扩展且原则性的方法。
