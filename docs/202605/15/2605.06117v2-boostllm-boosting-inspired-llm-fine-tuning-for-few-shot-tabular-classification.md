...
tags: ["query:llm-ts"]
score: 7.0
evidence: 针对结构化数据的 LLM 微调
tldr: 针对大语言模型在少样本表格分类任务中表现不如梯度提升决策树（GBDT）的问题，本文提出 BoostLLM 框架。该框架借鉴提升（Boosting）范式，将参数高效微调（PEFT）转化为多轮残差优化过程，通过训练序列化的适配器作为弱学习器，并结合决策树路径作为辅助输入。实验证明，BoostLLM 在多个数据集上显著优于标准微调，且在少样本场景下达到或超过了 XGBoost 的性能。
selection_source: fresh_fetch
motivation: 旨在解决大语言模型在处理少样本表格数据时，性能往往逊于传统梯度提升决策树模型的问题。
method: 提出 BoostLLM 框架，通过多轮残差优化训练序列化 PEFT 适配器，并引入决策树路径作为结构化特征增强输入。
result: 在多个模型和数据集上，BoostLLM 的表现一致优于标准微调，且在少样本情况下能与 XGBoost 媲美甚至超越 GPT-4o。
conclusion: 提升（Boosting）可以作为大语言模型微调的通用训练原则，特别是在处理结构化数据的低数据量场景中具有显著优势。
---
