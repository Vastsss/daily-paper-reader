...
title_zh: 基于大语言模型增强语义网络的跨股票可预测性
authors: "Yikuan Huang, Zheqi Fan, Kaiqi Hu, Yifan Ye"
date: 2026-04-21
pdf: "https://arxiv.org/pdf/2604.19476v2"
tags: ["query:llm-ts"]
score: 7.0
evidence: 利用LLM增强语义网络进行股票预测
tldr: 本研究针对基于文本的金融网络中存在的虚假连接问题，提出了一种利用大语言模型（LLM）增强语义网络的方法。该框架首先通过10-K文件嵌入构建候选图，再利用LLM识别并过滤缺乏经济关联的边。通过在标普500成分股上的回测，证明了该方法能有效提升跨股票收益预测的准确性，显著改善了夏普比率并降低了最大回撤，为金融网络分析提供了更具经济保真度的推理工具。
selection_source: fresh_fetch
motivation: 传统的基于文本相似度的金融网络往往包含大量虚假连接，因为文本接近并不等同于实际的经济关联。
method: 提出两阶段框架，先从10-K嵌入构建候选图，再利用LLM根据经济关系对边缘进行分类和过滤，并结合关系感知权重聚合交易信号。
result: "在2011至2019年标普500成分股的回测中，LLM过滤使夏普比率从0.742提升至0.820，最大回撤从-10.47%降至-7.85%。"
conclusion: LLM的推理能力能显著提高文本衍生金融网络的经济保真度，从而增强跨股票收益的可预测性。
---

## 摘要
基于文本的金融网络越来越多地被用于研究跨股票收益的可预测性。一种常见的方法是根据公司披露文件的嵌入向量相似性来构建连接，但此类网络通常包含伪边缘，因为文本上的接近并不一定意味着经济上的联系。我们提出了一个两阶段框架，首先从 10-K 文件的嵌入向量中构建稀疏候选图，然后利用大语言模型根据经济关系对候选边缘进行分类和过滤。优化后的图被用于将配对层面的均值回归信号聚合为股票层面的交易信号，并采用关系感知和基于距离的权重。在 2011 年至 2019 年标普 500 指数成份股的回测中，基于大语言模型的边缘过滤将多空夏普比率从 0.742 提高到 0.820，并将最大回撤从 -10.47% 降低到 -7.85%。这些结果表明，基于大语言模型的推理可以提高文本衍生金融网络的经济保真度，并增强跨股票的可预测性。
