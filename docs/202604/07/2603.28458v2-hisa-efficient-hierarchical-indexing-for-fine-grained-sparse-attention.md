...
tldr: 针对细粒度稀疏注意力（如DeepSeek的DSA）中索引器随上下文长度增加而产生的计算瓶颈，本文提出HISA。HISA通过两阶段分层索引（块级粗过滤和Token级精炼）替代原始的扁平扫描，在无需重新训练的情况下，显著提升了长文本处理速度，同时在DeepSeek-V3.2等模型上保持了与原始模型相当的性能。
selection_source: fresh_fetch
motivation: 现有的细粒度稀疏注意力机制在索引阶段需要扫描所有历史Token，导致计算开销随上下文长度增长而成为推理瓶颈。
method: 提出一种分层索引方法，先通过块级池化表示进行粗筛选，再在保留的候选块内进行细粒度的Token级索引。
result: 在64K上下文下实现了高达10倍的加速，且在Needle-in-a-Haystack等长文本任务中表现出与原始DSA相当的质量。
