...
score: 7.0
evidence: 动态时空图的联邦学习
tldr: 针对动态时空图联邦学习（FSTG）中存在的空间和时间异质性问题，本文提出了一种受因果启发的框架 SC-FSGL。该框架通过表示层面的干预，将可迁移的因果知识与客户端特定的噪声解耦。通过条件分离模块和因果代码簿，SC-FSGL 能够有效缓解表示纠缠和负迁移，在五个异质性时空图数据集上显著提升了泛化性能。
selection_source: fresh_fetch
motivation: 现有的联邦图学习方法忽略了时空异质性和客户端特定知识，导致虚假表示纠缠和负迁移，限制了在动态时空图上的泛化能力。
method: 提出 SC-FSGL 框架，利用条件分离模块进行软干预以解耦因果因子，并结合因果代码簿通过对比学习促进跨客户端的知识共享。
result: 在五个具有不同异质性的时空图数据集上的实验结果显示，SC-FSGL 的性能显著优于现有的最先进方法。
conclusion: 本研究证明了通过因果干预解耦不变特征与特定噪声，是解决联邦时空图学习中异质性挑战的有效途径。
---

## 摘要
联邦图学习 (FGL) 已成为在保护数据隐私的同时进行图神经网络去中心化训练的一种强大范式。然而，现有的 FGL 方法主要针对静态图设计，并依赖于参数平均或分布对齐，这隐含地假设所有特征在客户端之间具有同等的可迁移性，忽略了现实世界图中的时空异质性以及客户端特定知识的存在。在这项工作中，我们发现此类假设会导致伪表示纠缠、客户端特定干扰和负迁移的恶性循环，从而降低了动态时空图联邦学习 (FSTG) 的泛化性能。为了解决这一问题，我们提出了一种名为 SC-FSGL 的新型因果启发框架，该框架通过表示层面的干预，显式地将可迁移的因果知识与客户端特定的噪声解耦。具体而言，我们引入了一个条件分离模块，通过客户端条件掩码模拟软干预，从而实现不变时空因果因子与伪信号的解耦，并减轻由客户端异质性引起的表示纠缠。此外，我们提出了一个因果码本，通过聚类因果原型并通过对比学习对齐局部表示，从而促进跨客户端的一致性，并促进跨不同时空模式的知识共享。在五个具有不同异质性的时空图 (STG) 数据集上的实验表明，SC-FSGL 优于现有最先进的方法。
