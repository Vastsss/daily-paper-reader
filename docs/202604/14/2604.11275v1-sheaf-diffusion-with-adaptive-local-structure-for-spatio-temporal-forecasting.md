...
## 摘要
时空系统通常对局部扰动表现出高度异质且非直观的响应，这限制了传统消息传递方法在局部异质性下建模高阶交互的有效性。本文将时空预测重新表述为在局部结构化空间上学习信息流的问题，而非传播全局对齐的节点表示。我们引入了一种时空层状扩散图神经网络（ST-Sheaf GNN），它将图拓扑嵌入到由学习到的线性限制映射连接的层论向量空间中。与以往依赖静态或全局共享变换的工作不同，我们的模型学习随时间演化并适应局部时空模式的动态限制映射，从而实现更具表达力的交互。通过显式建模潜在的局部结构，所提出的框架有效地缓解了深度 GNN 架构中的过度平滑现象。我们在涵盖多个领域的各种真实世界时空预测基准数据集上评估了我们的框架。实验结果证明了其达到了最先进的性能，突显了层论拓扑表示作为时空图学习强大基础的有效性。代码可在以下网址获取：https://anonymous.4open.science/r/ST-SheafGNN-6523/。
