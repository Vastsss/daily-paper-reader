...
title_zh: 基于嵌入与生成式方法的大语言模型驱动文档分类评估：机遇与挑战
...
## 摘要
本研究对用于地球科学技术文档分类的基于嵌入的模型和生成式模型进行了对比分析。利用多学科基准数据集，我们评估了模型准确性、稳定性和计算成本之间的权衡。研究发现，通过思维链（CoT）提示增强的生成式视觉-语言模型（VLM，如 Qwen2.5-VL），其零样本准确率（82%）优于最先进的多模态嵌入模型（如 QQMM，63%）。我们还证明，虽然有监督微调（SFT）可以提高 VLM 的性能，但其对训练数据的不平衡较为敏感。
