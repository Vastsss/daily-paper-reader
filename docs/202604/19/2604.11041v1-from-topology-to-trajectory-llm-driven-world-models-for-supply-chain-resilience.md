...
tags: ["query:llm-ts"]
score: 7.0
evidence: 用于供应链轨迹演练的大语言模型驱动世界模型
tldr: 针对半导体供应链在政策黑天鹅事件下的韧性挑战，本文提出ReflectiChain框架。该框架通过生成式世界模型引入潜在轨迹演练，结合双重反思机制（行动中与行动后）与回顾性强化学习，解决了传统LLM在复杂宏观规划中的决策瘫痪和接地差距问题。在Semi-Sim基准测试中，该方法显著提升了极端场景下的操作率和奖励，实现了语义推理与物理现实的有效衔接。
selection_source: fresh_fetch
motivation: 传统LLM规划器在应对地缘政治引发的供应链黑天鹅事件时，因缺乏物理环境建模而面临决策瘫痪和接地差距。
method: 提出ReflectiChain框架，集成基于生成式世界模型的潜在轨迹演练、双重反思机制以及回顾性智能体强化学习。
result: "在极端场景下，ReflectiChain相比最强基线提升了250%的平均奖励，并将可操作率从13.3%提升至88.5%以上。"
conclusion: 物理接地约束与双重循环学习的协同作用是弥合语义推理与物理现实差距、实现长程战略规划的关键。
---
