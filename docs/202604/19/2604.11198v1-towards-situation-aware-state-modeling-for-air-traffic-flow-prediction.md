...
tags: ["query:llm-ts"]
score: 6.0
evidence: 航空交通的时间序列预测范式
tldr: 针对终端区空中交通流量预测，本文提出AeroSense框架。该框架摒弃了传统的宏观时间序列预测范式，直接对微观飞机状态进行建模，将其表示为动态状态集。通过态势感知表征和掩码自注意力机制捕捉飞机间交互，并利用解耦预测头处理不同功能区的流量动态。实验证明，该方法在预测精度、高峰期鲁棒性和可解释性方面均优于现有基准。
selection_source: fresh_fetch
motivation: 传统基于时间序列的预测方法将轨迹聚合为宏观序列，忽略了飞机实时运动学状态和空域边界关系等关键微观信息。
method: 提出一种直接从状态到流量的建模框架，利用态势感知表征和自注意力机制处理变长飞机状态集并捕捉个体间的相互影响。
result: 在真实大规模机场数据集上的实验显示，AeroSense在预测精度上达到SOTA，且在交通高峰期表现出更强的鲁棒性与可解释性。
conclusion: 研究证明直接建模微观飞机状态比传统时间序列方法具有更高的预测保真度，为主动式空中交通管理提供了更精准的决策支持。
---

## 摘要
终端管制区（TA）内准确的空中交通预测对于主动式空中交通管理（ATM）至关重要。然而，现有的数据驱动方法主要依赖于基于时间序列的预测范式，这本质上忽略了关键的航空器状态信息，例如实时运动学特征以及与空域边界的接近程度。为了解决这一局限性，我们提出了 AeroSense，这是一个用于空中交通预测的直接“状态到流量”建模框架。与先将航空器轨迹聚合为宏观流量序列再进行建模的经典时间序列方法不同，AeroSense 将实时空域态势显式地表示为“航空器状态的动态集合”，从而能够直接处理变数量的航空器输入，而非时间序列。具体而言，我们引入了一种情境感知状态表示，使 AeroSense 能够直接从微观航空器状态中感知瞬时终端区态势。此外，我们设计了一种模型架构，结合了掩码自注意力机制以捕捉航空器间的相互作用，并利用两个解耦的预测头来建模终端区内两个关键功能区域的异构流量动力学。在大型真实机场数据集上的广泛实验表明，AeroSense 持续达到最先进的性能，验证了直接对微观航空器状态建模比基于时间序列的基准模型具有显著更高的预测保真度。此外，所提出的框架在交通高峰期表现出卓越的鲁棒性，在分时段多目标评估下实现了帕累托最优性能，并通过基于注意力的可视化提供了有意义的可解释性。
