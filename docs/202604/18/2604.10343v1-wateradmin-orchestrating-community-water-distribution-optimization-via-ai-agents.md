...
tldr: 针对社区供水系统在动态环境（如天气、人类活动）下难以实时优化调度的问题，本文提出WaterAdmin框架。该框架采用双层架构，上层利用大语言模型（LLM）抽象复杂的社区背景信息，下层结合传统优化算法进行精确控制。通过在EPANET平台上的实验，证明了该方法在提高供水压力可靠性和降低能耗方面优于传统方法，实现了智能化与可靠性的结合。
selection_source: fresh_fetch
motivation: 传统优化方法难以实时处理和适应社区供水中高度动态且异构的背景信息（如天气和人类活动）。
method: 提出WaterAdmin双层框架，将LLM的背景理解能力与基于优化的底层控制逻辑相结合。
result: 在EPANET模拟平台上，WaterAdmin在维持压力可靠性和减少能源消耗方面表现出卓越的性能。
conclusion: 将AI智能体与传统优化相结合是解决复杂动态环境下社区资源调度问题的有效途径。
---

## 摘要
我们研究了社区供水系统的运行，其中必须对水泵和阀门进行调度，以在最小化能耗的同时可靠地满足用水需求。虽然现有的基于优化的方法在建模良好的环境下是有效的，但现实世界的社区场景表现出高度动态的背景——如人类活动、天气变化等——这些因素显著影响了不同区域的用水需求模式和运行目标。传统的优化方法难以实时聚合并适应这种异构且快速演变的背景信息。虽然大语言模型（LLM）智能体在理解异构社区背景方面具有强大能力，但它们并不适合直接生成可靠的实时控制动作。为了应对这些挑战，我们提出了一个基于 AI 智能体的双层框架 WaterAdmin，该框架在上层集成了基于 LLM 的社区背景抽象，在下层集成了基于优化的运行控制。这种设计利用了两种范式的互补优势，从而实现自适应且可靠的运行。我们在水力模拟平台 EPANET 上实现了 WaterAdmin，并证明了其在高度动态的社区背景下，在维持压力可靠性和降低能耗方面具有卓越性能。
