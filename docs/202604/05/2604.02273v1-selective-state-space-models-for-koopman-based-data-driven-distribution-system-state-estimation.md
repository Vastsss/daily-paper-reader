...
title_zh: 基于 Koopman 的数据驱动配电网状态估计的选择性状态空间模型
...
tldr: 针对现代配电网中分布式能源集成带来的状态估计（DSSE）难题，本文提出MambaDSSE框架。该方法结合了Koopman理论概率滤波与选择性状态空间模型（Mamba），实现了无模型的数据驱动推理。实验证明，该模型在处理长程依赖、应对DER渗透及采样不规律方面优于传统机器学习方法，显著提升了DSSE的扩展性和鲁棒性。
selection_source: fresh_fetch
motivation: 传统DSSE方法难以应对分布式能源带来的复杂性，且现有数据驱动方法在泛化性和可扩展性上存在局限。
method: 提出MambaDSSE框架，将Koopman理论的概率滤波与能够捕捉长程依赖的选择性状态空间模型（Mamba）相结合。
result: 实验表明该方法在可扩展性、对DER渗透的抗干扰能力以及对不规则采样率的鲁棒性方面均优于现有机器学习基准。
conclusion: MambaDSSE通过高效捕获系统时变行为的长程依赖，为复杂配电网的状态估计提供了一种高性能且稳健的解决方案。
---

## 摘要
由于分布式能源 (DER) 的接入，配电网状态估计 (DSSE) 在现代电网中发挥着日益重要的作用。配电网的固有特性使得经典估计方法难以应对，而数据驱动学习方法的最新进展虽然展现出潜力，但在泛化性和可扩展性方面存在系统性失效，限制了其适用性。在这项工作中，我们提出了 MambaDSSE，这是一个无模型的数据驱动框架，它将 Koopman 理论概率滤波与选择性状态空间模型相结合，通过学习从数据中推断系统的潜在时变行为。我们在多种测试系统和场景下对该模型进行了评估，结果表明，所提方法在可扩展性、对 DER 渗透水平的韧性以及对数据采样率不规则性的鲁棒性方面均优于机器学习基准方法。我们进一步强调了基于 Mamba 的状态空间模型 (SSM) 捕捉数据中长程依赖关系的能力，从而提升了 DSSE 任务的性能。
