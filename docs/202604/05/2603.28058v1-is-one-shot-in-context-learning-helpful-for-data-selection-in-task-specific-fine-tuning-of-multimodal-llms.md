...
tldr: 本研究针对多模态大模型（MLLM）在特定任务微调中面临的扩展性挑战，提出了 CLIPPER 框架。该框架是一种无需训练的数据选择流水线，通过单样本上下文学习（ICL）来探测模型对不同样本组合的响应。CLIPPER 能够识别出镜像原始数据集困惑度分布的核心子集，从而在保持数据多样性的同时保留关键样本。实验证明，该方法在显著降低计算成本和训练时间的前提下，能够达到与全量微调相当的性能，为高效 MLLM 微调提供了新思路。
selection_source: fresh_fetch
motivation: 现有数据选择方法依赖额外评分模型或启发式聚类，忽视了样本间的相互作用，且难以兼顾数据重要性与多样性。
method: 提出 CLIPPER 框架，利用单样本上下文学习探测模型对不同样本组合的响应，通过模拟原始数据集的困惑度分布来筛选核心子集。
result: "在多个模型和数据集上，CLIPPER 仅需部分数据即可达到全量微调的效果，如在 ScienceQA 上减少了 37% 的训练时间。"
conclusion: 证明了利用上下文学习进行数据选择能有效提升多模态大模型微调的效率，在保持性能的同时显著降低计算开销。
---
