...
tags: ["query:llm"]
score: 6.0
evidence: 基于LLM的多智能体系统与通信拓扑
tldr: 本研究探讨了基于大语言模型的多智能体系统（MAS）中通信拓扑的隐私风险，提出了通信推断攻击（CIA）。该方法在黑盒设置下通过构造对抗性查询诱导智能体产生推理输出，并利用全局偏差解耦和LLM引导的弱监督技术建模语义相关性，从而推断内部通信结构。实验证明CIA能以极高准确率识别拓扑，揭示了MAS在知识产权和系统安全方面的重大隐患。
selection_source: fresh_fetch
motivation: 旨在揭示黑盒环境下多智能体系统通信拓扑被恶意推断的隐私风险及其对知识产权的威胁。
method: 提出CIA攻击框架，通过对抗性查询获取推理输出，并结合全局偏差解耦与弱监督学习建模智能体间的语义关联。
result: 在多种优化拓扑的MAS上，CIA达到了0.87的平均AUC，最高峰值达0.99，展现了极强的推断能力。
conclusion: 研究证明了多智能体系统的通信拓扑存在严重的隐私泄露风险，亟需加强相关的安全防护措施。
---

## 摘要
基于大语言模型的多智能体系统（MAS）在解决复杂任务方面展现出了卓越的能力。通信拓扑是 MAS 的核心，它决定了智能体之间如何进行内部信息交换。因此，通信拓扑的安全性引起了越来越多的关注。在本文中，我们研究了一个关键的隐私风险：在受限的黑盒设置下，MAS 的通信拓扑可以被推断出来，从而暴露系统漏洞并构成重大的知识产权威胁。为了探索这一风险，我们提出了通信推断攻击（CIA），这是一种新型攻击方法，通过构建新的对抗性查询来诱导中间智能体的推理输出，并通过所提出的全局偏差解耦和 LLM 引导的弱监督来建模它们的语义相关性。在具有优化通信拓扑的 MAS 上进行的广泛实验证明了 CIA 的有效性，其平均 AUC 达到 0.87，峰值 AUC 高达 0.99，从而揭示了 MAS 中存在的重大隐私风险。
