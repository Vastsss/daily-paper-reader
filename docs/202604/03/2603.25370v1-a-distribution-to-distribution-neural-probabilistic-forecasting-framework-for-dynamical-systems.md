...
tldr: 针对动力系统概率预测中传统方法依赖轨迹采样或集成模拟的局限，本文提出一种分布到分布（D2D）的神经预测框架。该框架利用核均值嵌入表示输入分布，并通过混合密度网络参数化输出分布，实现了预测不确定性在端到端架构中的直接递归演化。在Lorenz63系统上的实验证明，该方法无需显式集成模拟即可捕捉非线性动力学下的分布演化，具有较强的预测能力。
selection_source: fresh_fetch
motivation: 现有的概率预测方法多基于轨迹采样或集成，缺乏直接将预测分布作为动力学对象进行演化的能力。
method: 提出D2D框架，结合核均值嵌入进行分布编码和混合密度网络进行解码，实现分布的直接映射与递归传播。
result: 在Lorenz63混沌系统上，该模型能准确捕捉复杂的分布演化，且在预测技巧上优于或等同于简化的完美模型基准。
conclusion: 该研究为概率预测提供了一种新范式，证明了直接学习和演化预测分布比通过集成间接重构更具潜力。
---

## 摘要
概率预测通过将预测表示为概率分布而非确定性轨迹，为动力系统中的不确定性量化提供了一个原则性框架。然而，现有的预测方法，无论是基于物理的还是基于神经网络的，在根本上仍然是面向轨迹的：预测分布通常通过集合或采样获得，而不是作为动力学对象直接演化。本文开发了一种分布到分布（D2D）神经概率预测框架，直接对预测分布进行操作。该框架围绕一个可替换的神经预测模块引入了分布编码和解码结构，利用核均值嵌入表示输入分布，并使用混合密度网络对输出预测分布进行参数化。这种设计使得预测不确定性能够在统一的端到端神经架构中进行递归传播，且模型的训练和评估直接根据概率预测技巧进行。该框架在 Lorenz63 混沌动力系统上进行了演示。结果表明，D2D 模型能够捕捉非线性动力学下非平凡的分布演化，在无需显式集合模拟的情况下产生高质量的概率预测，并且与简化的完美模型基准相比具有竞争力，在某些情况下甚至表现更优。这些发现指向了一种概率预测的新范式，即预测分布是直接学习和演化的，而不是通过基于集合的不确定性传播间接重构的。
