...
tldr: 针对大语言模型在处理重复或近似查询时推理成本高昂的问题，本文提出 MemBoost 框架。该框架通过轻量级模型复用历史生成答案并检索相关信息实现低成本推理，同时利用成本感知路由将难题转交给强模型。MemBoost 支持持续内存增长和交互式场景，在保证回答质量的同时显著降低了推理开销。
selection_source: fresh_fetch
motivation: 解决大语言模型在处理跨用户和会话的重复或相似查询时，推理成本过高且资源浪费的问题。
method: 提出一种结合轻量级模型、历史答案复用机制及成本感知路由的框架，实现按需调用强模型。
result: 实验表明该框架大幅减少了昂贵大模型的调用次数，在显著降低成本的同时保持了与强模型相当的回答质量。
conclusion: MemBoost 为交互式场景下的低成本、高性能 LLM 服务提供了一种有效的内存增强型解决方案。
---

## 摘要
大语言模型（LLMs）表现出强大的性能，但在实际服务中会产生高昂的推理成本，特别是在跨用户和会话存在重复或近乎重复查询的工作负载下。在这项工作中，我们提出了 MemBoost，这是一个记忆增强的大语言模型服务框架，它使轻量级模型能够重用先前生成的答案并检索相关的支持信息以进行低成本推理，同时选择性地将困难或不确定的查询升级到更强大的模型。与主要针对单次响应提供依据的标准检索增强生成（RAG）不同，MemBoost 通过支持答案重用、持续记忆增长和成本感知路由，专为交互式场景设计。在模拟工作负载下对多个模型进行的实验表明，MemBoost 显著减少了昂贵的大模型调用和整体推理成本，同时保持了与强模型基准相当的高回答质量。
