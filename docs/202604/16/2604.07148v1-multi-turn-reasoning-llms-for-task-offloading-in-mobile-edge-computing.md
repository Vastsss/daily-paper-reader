...
tags: ["query:llm-ts"]
score: 6.0
evidence: 用于动态任务卸载的多轮推理LLM
tldr: 针对移动边缘计算中任务卸载决策的动态性与复杂性，本文提出COMLLM框架。该框架利用大语言模型的推理能力，结合GRPO算法与前瞻性协作模拟机制，通过多步蒙特卡洛模拟捕捉决策的长期影响。COMLLM不仅实现了近乎最优的延迟和负载均衡，还具备卓越的零样本拓扑扩展性，能直接应用于未见过的更大规模网络，克服了传统强化学习方法泛化性差且需重新训练的局限。
selection_source: fresh_fetch
motivation: 传统启发式方法适应性差，而深度强化学习在网络拓扑变化时缺乏泛化性，且标准LLM微调难以兼顾长期系统演化。
method: 提出COMLLM框架，通过集成GRPO优化算法与前瞻性协作模拟（LACS）机制，实现基于多步预测的远见型任务卸载决策。
result: 实验证明COMLLM在降低延迟和负载均衡方面表现优异，且无需重新训练即可在更大规模的未知网络拓扑中实现零样本扩展。
conclusion: 结合多轮推理与强化学习的LLM为解决MEC复杂决策问题提供了新路径，在性能、泛化性和拓扑灵活性上均优于传统基准方法。
---
