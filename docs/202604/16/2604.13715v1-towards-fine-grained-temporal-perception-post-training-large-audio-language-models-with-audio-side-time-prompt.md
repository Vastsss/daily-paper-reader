...
title_zh: 迈向细粒度时间感知：利用音频侧时间提示对大规模音频-语言模型进行后训练
...
tags: ["query:llm-ts"]
score: 8.0
evidence: 大型音频语言模型中的细粒度时间感知
tldr: 针对大语言音频模型在细粒度时间感知（如事件起止时间推断）方面的不足，本文提出了TimePro-RL框架。该框架通过将时间戳编码为嵌入向量并交织在音频特征序列中作为“音频侧时间提示”，并结合监督微调与强化学习来直接优化时间对齐性能。实验表明，该方法在音频定位、声音事件检测和密集音频描述等任务上均取得了显著的性能提升，验证了其在增强模型时间感知能力方面的有效性。
selection_source: fresh_fetch
motivation: 现有大语言音频模型在推断音频事件起止时间等细粒度时间感知任务上表现欠佳，限制了其实际应用价值。
method: 提出TimePro-RL框架，将时间戳嵌入作为音频侧提示交织于特征序列中，并引入强化学习以直接优化模型的时间对齐性能。
result: 实验证明TimePro-RL在音频定位、声音事件检测和密集音频描述等多种时间感知任务中均实现了显著的性能增长。
conclusion: 通过结合音频侧时间提示与强化学习优化，可以显著提升大语言音频模型对音频事件细粒度时间边界的感知与定位能力。
---

## 摘要
大规模音频-语言模型（LALMs）实现了通用的音频理解，并在各种音频任务中展现出卓越的性能。然而，这些模型在时间感知方面（例如推断事件的开始和结束时间）仍面临挑战，导致其在细粒度场景中的实用性受限。为解决这一问题，我们提出了音频侧时间提示（Audio-Side Time Prompt），并利用强化学习（RL）开发了用于细粒度时间感知的 TimePro-RL 框架。具体而言，我们将时间戳编码为嵌入向量，并将其交错插入音频特征序列中作为时间坐标来提示模型。此外，我们在有监督微调（SFT）之后引入了强化学习，以直接优化时间对齐性能。实验表明，TimePro-RL 在音频定位（audio grounding）、声音事件检测和密集音频描述（dense audio captioning）等一系列音频时间任务中取得了显著的性能提升，验证了其强大的有效性。
