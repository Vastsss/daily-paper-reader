...
tags: ["query:llm-ts"]
score: 7.0
evidence: 用于大气预测的视觉Transformer
tldr: 针对AI数值天气预报中高分辨率输出计算开销大及多尺度适应性差的问题，本文提出GSSA-ViT框架。该方法首次将3D高斯泼溅（3DGS）引入气象领域，将经纬度网格视为高斯中心，通过生成式方案预测高斯参数，并结合尺度感知注意力机制实现任意分辨率的预报与降尺度。在ERA5和CMIP6数据集上的实验证明了其在多尺度大气预测中的高效性与准确性。
selection_source: fresh_fetch
motivation: 现有的AI天气预报模型在处理高分辨率输出时面临计算效率低和多尺度适应性不足的挑战。
method: 提出GSSA-ViT框架，利用生成式3D高斯建模表示大气场，并设计尺度感知注意力模块捕捉跨尺度依赖。
result: 在ERA5数据集上实现了87个变量的任意分辨率预报，并在ERA5和CMIP6的降尺度任务中表现优于现有方法。
conclusion: 该框架为高分辨率、多尺度的大气预测和降尺度提供了一种高效且可扩展的创新解决方案。
---
