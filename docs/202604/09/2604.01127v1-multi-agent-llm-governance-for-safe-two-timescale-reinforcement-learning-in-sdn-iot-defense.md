...
tldr: 本文针对SDN-IoT防御中缓解措施可能导致控制器过载和QoS下降的问题，提出了一种双时间尺度防御方案。该方案在快速尺度利用PPO智能体进行受限缓解，在慢速尺度利用多智能体大语言模型（LLM）治理引擎动态更新全局策略章程，实现了无需重新训练的可审计策略演进，显著提升了防御性能并降低了系统风险。
selection_source: fresh_fetch
motivation: 现有的SDN-IoT防御方法往往忽视了缓解动作对控制器负载和系统稳定性的负面影响，且缺乏可审计的策略演进机制。
method: 提出一种结合快速尺度PPO缓解智能体与慢速尺度LLM治理引擎的双时间尺度架构，通过LLM生成并验证可解析的策略更新。
result: "实验显示该方法在Macro-F1上优于PPO 9.1%，同时大幅降低了控制器积压峰值和灾难性过载率。"
conclusion: 通过引入LLM治理机制，可以在保证SDN-IoT网络安全的同时，有效维护控制平面的稳定性和服务质量。
---

## 摘要
软件定义网络 (SDN) 因其集中控制和可编程转发特性，正越来越多地被用于保障物联网 (IoT) 网络的安全。然而，SDN-IoT 防御本质上是一个闭环控制问题，其中缓解措施会影响控制器负载、队列动态、规则安装延迟以及未来的流量观测。激进的缓解措施可能会导致控制平面不稳定、降低服务质量 (QoS) 并放大系统性风险。现有的基于学习的方法优先考虑检测准确性，却忽略了控制器耦合以及缺乏结构化、可审计策略演进的短视界强化学习 (RL) 优化。本文提出了一种自我反思的双时间尺度 SDN-IoT 防御方案，将快速缓解与慢速策略治理分离。在快时间尺度上，每个交换机的近端策略优化 (PPO) 智能体在安全约束和动作掩码下执行感知控制器的缓解操作。在慢时间尺度上，多智能体大语言模型 (LLM) 治理引擎生成全局策略章程 Pi 的机器可解析更新，该章程编码了允许的动作、安全阈值和奖励优先级。更新 (Delta Pi) 通过压力测试进行验证，且仅在具备回归测试和安全保证的情况下部署，从而在无需重新训练 RL 智能体的情况下确保可审计的演进。在异构 IoT 流量和对抗性压力下的评估显示，其 Macro-F1 分数比 PPO 提高了 9.1%，比静态基准提高了 15.4%。在高强度攻击下，最坏情况下的性能下降减少了 36.8%，控制器积压峰值降低了 42.7%，且 RTT p95 膨胀保持在 5.8% 以下。策略演进在五个周期内收敛，将灾难性过载从 11.6% 降低到 2.3%。
