...
score: 6.0
evidence: 时间定位与跨模态学习
tldr: 针对视频时刻句子定位（TSGV）任务中预训练视频编码器与定位任务不匹配的问题，本文提出了一种全端到端训练范式。通过引入句子条件适配器（SCADA），利用文本特征自适应地优化视频骨干网络的一小部分参数。该方法在降低内存消耗的同时，通过语言嵌入动态调制特征图，显著增强了视觉表示能力，在两大基准测试上均达到了SOTA水平。
selection_source: fresh_fetch
motivation: 现有的TSGV方法多采用冻结的预训练视频编码器进行离线特征提取，导致视频分类预训练任务与定位任务之间存在显著的任务差异。
method: 提出全端到端训练范式，并设计了句子条件适配器（SCADA），通过语言嵌入动态调制视频特征图以实现高效的骨干网络微调。
result: 实验结果表明，该方法在两个主流基准测试上均超越了现有的最先进技术，证明了端到端学习和SCADA模块的有效性。
conclusion: 全端到端训练结合轻量化适配器能有效解决特征提取与下游任务脱节的问题，是提升视频时刻句子定位性能的关键。
---

## 摘要
视频时序句子定位（TSGV）旨在从一段未剪辑的视频中定位出与句子查询语义相对应的时序片段。目前大多数方法采用预训练的、与查询无关的视觉编码器进行离线特征提取，且视频主干网络是冻结的，并未针对 TSGV 任务进行优化。这导致了针对视觉分类训练的视频主干网络在应用于 TSGV 任务时存在任务差异问题。为了弥补这一差距，我们提出了一种全端到端范式，共同优化视频主干网络和定位头。我们首先进行了一项实证研究，验证了在不同模型规模下，端到端学习相较于冻结基准模型的有效性。此外，我们引入了一种句子条件适配器（SCADA），它利用句子特征来自适应地训练视频主干网络中的一小部分参数。SCADA 能够以较低的内存消耗部署更深的网络主干，并通过精确整合语言嵌入来调制特征图，从而显著增强视觉表示。在两个基准测试上的实验表明，我们的方法优于现有最先进的方法。代码和模型将会发布。
