...
tldr: 本文针对重尾时间序列提出了一种基于特征函数的参数化广义谱族，旨在解决非高斯平稳过程的分布特征刻画问题。该方法无需矩假设即可捕捉非线性依赖关系，支持对柯西自回归及离散稳定整数值自回归模型进行一致性参数估计。通过集成周期图避免了平滑参数的影响，能自动识别模型的因果性结构，并在拟合优度检验和麻疹病例预测中展现了优越的性能。
selection_source: fresh_fetch
motivation: 传统频谱分析方法在处理缺乏矩假设的重尾时间序列和非线性依赖关系时存在局限性。
method: 引入基于特征函数的参数化广义谱族，并利用集成周期图开发了无需平滑参数的推断程序。
result: 实现了对重尾模型（如柯西AR和离散稳定INAR）的根号n一致性估计，并能有效区分因果与非因果结构。
conclusion: 该研究为重尾和非线性时间序列提供了稳健的频域分析框架，在统计检验和实际数据预测中具有显著的应用价值。
---
