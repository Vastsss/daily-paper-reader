...
tags: ["query:llm-ts"]
score: 7.0
evidence: 生成混合类型时间序列数据
tldr: 针对电子健康记录（EHR）合成中的隐私保护和混合类型时间序列建模难题，本文提出CDMT-EHR框架。该框架采用连续时间扩散模型，结合双向GRU捕捉时序依赖，并通过可学习嵌入将类别变量统一到高斯扩散过程中。此外，引入因子化可学习噪声调度，显著提升了生成质量与效率，在保持高保真度的同时将采样步数从1000步降至50步，并支持条件生成。
selection_source: fresh_fetch
motivation: 现有的离散时间扩散模型在处理混合类型时序EHR时存在近似误差大、采样效率低且难以有效建模跨特征关联的问题。
method: 提出一种基于Bi-GRU骨干的连续时间扩散框架，利用可学习嵌入统一处理数值与类别特征，并采用因子化噪声调度优化学习过程。
result: 在两个大型ICU数据集上的实验表明，该方法在下游任务性能和分布保真度上均优于现有模型，且采样速度提升了20倍。
conclusion: CDMT-EHR为高质量、高效率的混合类型时序医疗数据合成提供了有效方案，并支持不平衡临床场景下的条件生成。
---

## 摘要
电子健康记录（EHR）对于临床研究具有极高的价值，但隐私问题严重限制了数据共享。合成数据生成提供了一个极具前景的解决方案，但 EHR 带来了独特的挑战：它们包含随时间演变的数值型和类别型特征。虽然扩散模型在 EHR 合成中表现出强大的性能，但现有方法主要依赖于离散时间公式，这面临着有限步近似误差以及训练与采样步数耦合的问题。我们提出了一种用于生成混合类型时间序列 EHR 的连续时间扩散框架，其贡献包括三个方面：(1) 采用双向门控循环单元（BiGRU）作为骨干网络的连续时间扩散，用于捕捉时间依赖性；(2) 通过类别变量的可学习连续嵌入实现统一的高斯扩散，从而实现跨特征的联合建模；(3) 一种分解的可学习噪声调度方案，能够适应每个特征在每个时间步的学习难度。在两个大规模重症监护室（ICU）数据集上的实验表明，我们的方法在下游任务性能、分布保真度和可区分性方面优于现有方法，且仅需 50 个采样步，而基线方法则需要 1000 步。无分类器指导（Classifier-free guidance）进一步实现了针对类别不平衡临床场景的有效条件生成。
