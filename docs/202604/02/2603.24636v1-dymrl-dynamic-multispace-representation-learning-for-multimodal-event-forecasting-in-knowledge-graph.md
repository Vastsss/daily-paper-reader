...
tldr: 本研究针对多模态知识图谱中的事件预测问题，提出了DyMRL框架。该框架解决了现有方法在动态知识获取和融合方面的不足，通过在欧几里得、双曲和复数空间中集成时间敏感的结构特征，并结合预训练模型的视觉与语言智能，捕捉深层关系感知特征。此外，引入的双重融合-演化注意力机制实现了多模态特征的动态加权融合。实验表明，DyMRL在四个新构建的基准数据集上均优于现有最先进方法。
selection_source: fresh_fetch
motivation: 现有研究多关注静态设置，难以有效获取动态多模态知识并捕捉不同模态对未来事件随时间变化的贡献。
method: 提出DyMRL框架，利用多空间关系消息传递学习深层结构特征，并采用双重融合-演化注意力机制进行动态多模态融合。
result: 在四个新构建的多模态时序知识图谱基准上，DyMRL的性能显著优于现有的动态单模态和静态多模态基准方法。
conclusion: DyMRL通过多空间表示学习和动态融合机制，有效提升了多模态时序知识图谱中的事件预测准确性。
---

## 摘要
准确的多模态知识表示对于现实场景中的事件预测至关重要。然而，现有研究主要集中在静态设置上，忽视了多模态知识的动态获取与融合。1) 在知识获取层面，如何学习不同模态的时敏信息，特别是动态结构模态。现有的动态学习方法通常局限于异构空间或简单单一空间中的浅层结构，难以捕捉深层的关系感知几何特征。2) 在知识融合层面，如何学习演化的多模态融合特征。基于静态协同注意力的现有知识融合方法难以捕捉不同模态对未来事件随时间变化的历史贡献。为此，我们提出了 DyMRL，一种动态多空间表示学习方法，旨在高效地获取和融合多模态时序知识。1) 针对前一问题，DyMRL 将来自欧几里得空间、双曲空间和复数空间的特定时间结构特征集成到关系消息传递框架中以学习深层表示，反映了人类在联想思维、高阶抽象和逻辑推理方面的智能。预训练模型赋予了 DyMRL 时敏的视觉和语言智能。2) 针对后一问题，DyMRL 结合了先进的双重融合-演化注意力机制，以对称方式在不同时间戳为不同模态分配动态学习权重。为了通过利用历史中学习到的多模态时序知识来评估 DyMRL 的事件预测性能，我们构建了四个多模态时序知识图谱基准。大量实验表明，DyMRL 优于最先进的动态单模态和静态多模态基准方法。
