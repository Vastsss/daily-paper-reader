...
motivation: 大语言模型在处理长文档列表重排序时，由于上下文过长导致效率低下且难以捕捉关键排序信息。
method: 提出一种与下游排序目标对齐的逐点摘要模型 SumRank，通过监督微调和强化学习训练，将长文档压缩为排序对齐的摘要。
result: 在 TREC DL 19-23 五个基准数据集上，SumRank 实现了最先进的排序性能，并大幅减少了摘要生成和重排序的计算复杂度。
conclusion: 通过将摘要模型与排序目标对齐，可以有效解决长文档重排序中的效率与效果平衡问题。
---
