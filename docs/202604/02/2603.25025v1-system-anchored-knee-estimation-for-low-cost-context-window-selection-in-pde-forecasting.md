...
tldr: "针对自回归神经偏微分方程（PDE）模拟器中上下文窗口选择成本高且缺乏规范化的问题，本文提出了SAKE方法。该方法通过结合物理可解释的系统锚点和拐点检测技术，在保证预测性能的同时大幅降低了搜索成本。在PDEBench基准测试中，SAKE实现了94.9%的搜索成本节省，是目前低成本窗口选择的最优方案。"
selection_source: fresh_fetch
motivation: 现有的PDE预测上下文窗口选择方法在计算成本、鲁棒性或与下游性能的对齐方面存在不足，缺乏高效的规范化选择机制。
method: 提出SAKE两阶段法，先利用物理系统锚点确定候选集，再通过拐点感知策略在候选集中进行最终的窗口大小选择。
result: "在PDEBench的多个任务中，SAKE以仅5.1%的成本实现了极高的选择准确率，并节省了约95%的搜索计算开销。"
conclusion: SAKE证明了结合物理启发式锚点与拐点估计是优化神经PDE模拟器上下文窗口选择的一种低成本且高效的途径。
---

## 摘要
自回归神经 PDE 模拟器根据有限的历史记录逐步预测物理场的演化，但此类模拟器的低成本上下文窗口选择仍是一个未形式化的问题。现有的时间序列预测上下文窗口选择方法包括穷举验证、直接低成本搜索和系统论记忆估计，但它们要么成本高昂、脆弱，要么与下游的外推（rollout）性能不直接对齐。我们将固定窗口自回归神经 PDE 模拟器的显式上下文窗口选择形式化为一个独立的低成本算法问题，并提出了系统锚定拐点估计（SAKE）。这是一种两阶段方法，首先从具有物理可解释性的系统锚点中识别出一个小型结构化候选集，然后在其中进行感知拐点的下游选择。在共享 L ∈ {1, ..., 16} 协议下评估的所有八个 PDEBench 族中，SAKE 是评估方法中整体表现最强的预算匹配低成本选择器，实现了 67.8% 的准确率（Exact）、91.7% 的 Within-1、6.1% 的平均拐点遗憾值（mean regret@knee）以及 0.051 的成本比（节省了 94.9% 的归一化搜索成本）。
