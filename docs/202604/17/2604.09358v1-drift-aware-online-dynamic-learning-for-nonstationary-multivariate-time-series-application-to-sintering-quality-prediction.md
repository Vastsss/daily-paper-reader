...
title_zh: 针对非平稳多元时间序列的漂移感知在线动态学习：在烧结质量预测中的应用
...
tldr: 针对工业烧结等复杂系统中非平稳多元时间序列预测面临的概念漂移和标签延迟挑战，本文提出了一种漂移感知的多尺度动态学习框架（DA-MSDL）。该框架通过多尺度双支路卷积网络提取时空特征，利用最大均值差异（MMD）实现无监督漂移检测，并结合分层微调与经验回放机制进行在线自适应更新，有效提升了模型在动态环境下的预测鲁棒性与泛化能力。
selection_source: fresh_fetch
motivation: 工业生产中非平稳数据流导致的概念漂移和标签反馈延迟，使得传统离线模型难以维持长期的预测准确性。
method: 提出DA-MSDL框架，集成多尺度特征提取、基于MMD的无监督漂移检测以及漂移程度引导的分层微调策略。
result: 在真实工业烧结及公开数据集上的实验证明，该框架在处理严重概念漂移时显著优于现有基准方法。
conclusion: 该研究为非平稳工业环境下的质量监控提供了一种能够兼顾稳定性与塑性的在线动态学习新范式。
---

## 摘要
准确预测非平稳多元时间序列在铁矿石烧结等复杂工业系统中仍是一项关键挑战。在实践中，显著的概念漂移加上严重的标签验证延迟，会导致离线训练模型的性能迅速下降。现有的基于静态架构或被动更新策略的方法，在缺乏即时监督的情况下，难以同时提取多尺度时空特征并克服稳定性-塑性困境。为了解决这些局限性，本文提出了一种漂移感知多尺度动态学习（DA-MSDL）框架，通过非平稳数据流上的在线自适应机制来维持稳健的多输出预测性能。该框架采用多尺度双分支卷积网络作为骨干，将局部波动与长期趋势解耦，从而增强对复杂动态模式的表征能力。为了规避标签延迟瓶颈，DA-MSDL 利用最大均值差异（MMD）进行无监督漂移检测。通过量化特征分布中的在线统计偏差，DA-MSDL 在推理前主动触发模型自适应。此外，本文还开发了一种由漂移严重程度引导的分层微调策略。在动态记忆队列的优先级经验回放支持下，该方法在实现快速分布对齐的同时，有效缓解了灾难性遗忘。在真实工业烧结数据和公开基准数据集上的长周期实验表明，DA-MSDL 在严重概念漂移下始终优于代表性基准模型。该框架展现出强大的跨领域泛化能力和预测稳定性，为非平稳环境下的质量监测提供了一种有效的在线动态学习范式。
