...
tldr: 针对强化学习智能体难以在相似任务间泛化的问题，本文提出ASPECT框架。该方法利用文本条件VAE和LLM作为动态语义算子，在测试时将新任务描述重映射为源任务描述，从而生成与原始策略兼容的“想象状态”以实现策略复用。这种方法摆脱了离散类别限制，在多种复杂类比任务中实现了高效的零样本迁移。
selection_source: fresh_fetch
motivation: 现有的零样本迁移方法受限于预定义的离散类别系统，难以应对新颖或复杂的任务变体。
method: 利用LLM作为语义算子将新任务描述重映射至源任务，并结合文本条件VAE生成可供原始策略执行的想象状态。
result: 在多种复杂且新颖的类比任务中，该方法成功实现了超越固定类别映射限制的零样本策略迁移。
conclusion: 结合LLM的推理能力与生成模型，ASPECT为强化学习提供了一种更通用、灵活的语义策略迁移新范式。
---
