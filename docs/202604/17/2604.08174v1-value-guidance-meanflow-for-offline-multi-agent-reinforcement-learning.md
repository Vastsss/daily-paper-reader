...
tldr: 针对离线多智能体强化学习中生成式策略采样效率低且对正则化系数敏感的问题，本文提出VGM^2P框架。该方法利用全局优势值引导智能体协作，将最优策略学习转化为条件行为克隆。通过引入MeanFlow模型和无分类器引导技术，在提升策略表达能力的同时实现了高效的一步动作生成。实验证明，该方法在多种任务中达到了SOTA性能，且具有极高的推理效率。
selection_source: fresh_fetch
motivation: 现有的基于扩散或流模型的离线MARL方法采样效率低，且性能高度依赖于行为正则化系数的调节。
method: 提出VGM^2P框架，利用全局优势值引导条件行为克隆，并结合MeanFlow实现高效的策略训练与推理。
result: 在离散和连续动作空间的任务中，VGM^2P仅通过条件行为克隆即可达到与当前最先进方法相当的性能。
conclusion: VGM^2P提供了一种简单高效的流式策略学习方案，有效平衡了离线MARL的性能与推理效率。
---

## 摘要
离线多智能体强化学习 (MARL) 旨在从预先收集的数据集中学习最优联合策略，这要求在最大化全局收益与缓解离线数据带来的分布偏移之间进行权衡。近期的研究利用扩散模型或流生成模型来捕捉智能体之间复杂的联合策略行为；然而，这些方法通常依赖多步迭代采样，从而降低了训练和推理效率。尽管后续研究通过蒸馏等方法提高了采样效率，但其对行为正则化系数仍然敏感。为了解决上述问题，我们提出了价值引导多智能体 MeanFlow 策略 (VGM$^2$P)，这是一个简单且有效的基于流的策略学习框架，能够通过对系数不敏感的条件行为克隆实现高效的动作生成。具体而言，VGM$^2$P 利用全局优势值来引导智能体协作，将最优策略学习视为条件行为克隆。此外，为了提高多智能体场景下的策略表达能力和推理效率，它在策略训练和执行中均采用了无分类器引导的 MeanFlow。在离散和连续动作空间任务上的实验表明，即使仅通过条件行为克隆进行训练，VGM$^2$P 也能高效地达到与当前最先进方法相当的性能。
