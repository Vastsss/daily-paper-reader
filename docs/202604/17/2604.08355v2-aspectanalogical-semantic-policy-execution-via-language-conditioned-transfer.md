...
method: 利用LLM动态地将当前任务描述重映射为源任务描述，并结合文本条件VAE生成原始策略可执行的想象状态。
result: 在多种复杂且新颖的类比任务中成功实现了零样本迁移，表现出超越固定类别映射的灵活性。
conclusion: 通过引入LLM的语义推理能力，ASPECT为强化学习提供了一种更具通用性和适应性的类比迁移新范式。
---
