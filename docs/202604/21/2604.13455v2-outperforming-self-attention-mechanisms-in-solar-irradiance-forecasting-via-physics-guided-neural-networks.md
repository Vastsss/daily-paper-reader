...
## 摘要
准确的全球水平辐照度（GHI）预测对于电网稳定性至关重要，尤其是在气溶胶波动剧烈的干旱地区。尽管近期的趋势倾向于计算成本高昂的基于 Transformer 的架构，但本文挑战了盛行的“复杂度优先”范式。我们提出了一种轻量级的物理告知混合 CNN-BiLSTM 框架，该框架优先考虑领域知识而非架构深度。该模型集成了用于空间特征提取的卷积神经网络（CNN）和用于捕捉时间依赖性的双向 LSTM（Bi-LSTM）。与标准的数据驱动方法不同，我们的模型由包含晴空指数和太阳天顶角在内的 15 个工程特征向量显式引导，而非仅仅依赖于原始历史数据。超参数通过贝叶斯优化进行严格调优，以确保全局最优性。在苏丹使用 NASA POWER 数据进行的实验验证表明，我们的物理引导方法实现了 19.53 W/m² 的均方根误差（RMSE），显著优于复杂的基于注意力的基准模型（RMSE 为 30.64 W/m²）。这些结果证实了“复杂度悖论”：在高噪声气象任务中，显式的物理约束为自注意力机制提供了一种更高效、更准确的替代方案。研究结果倡导向混合型、物理感知的人工智能转变，以实现实时可再生能源管理。
