...
date: 2026-04-13
pdf: "https://arxiv.org/pdf/2604.11807v3"
tags: ["query:llm"]
score: 7.0
evidence: 离网系统的太阳能辐照度预测
tldr: 针对离网光伏系统对高效且符合物理规律的太阳辐照度预测需求，本文提出物理信息状态空间模型（PISSM）。该模型通过动态Hankel矩阵嵌入过滤传感器噪声，利用线性状态空间模型替代复杂注意力机制以提升计算效率，并引入物理信息门控机制确保预测严格遵循昼夜更替规律。在苏丹数据集上的验证表明，PISSM以极低的参数量实现了高精度预测，为边缘侧实时控制提供了轻量化方案。
selection_source: fresh_fetch
motivation: 现有深度学习模型计算开销大且缺乏物理约束，难以在资源受限的离网系统微控制器上实现准确且符合物理常识的实时预测。
method: 结合动态Hankel矩阵嵌入进行去噪，采用线性状态空间模型建模时间依赖，并引入基于太阳高度角和清洁度指数的物理信息门控机制来约束输出范围。
result: 在苏丹多年份数据集上的实验证明，PISSM在参数量少于4万的情况下达到了卓越的预测精度，显著优于传统深度学习模型。
conclusion: PISSM成功平衡了计算效率与物理准确性，为离网系统边缘侧的实时太阳能预测树立了超轻量化且可靠的技术标杆。
---

## 摘要
离网光伏系统的稳定运行需要准确且计算高效的太阳能预测。当代的深度学习模型通常面临巨大的计算开销和物理盲目性，从而产生物理上不可能的预测结果。本文引入了物理信息状态空间模型（PISSM），旨在为边缘部署的微控制器弥合效率与物理准确性之间的差距。PISSM 利用动态汉克尔矩阵（Hankel matrix）嵌入，通过将原始气象序列转换为鲁棒的状态空间，来过滤随机传感器噪声。线性状态空间模型取代了沉重的注意力机制，能够高效地对时间依赖性进行建模以实现并行处理。至关重要的是，一种新型的物理信息门控机制利用太阳天顶角和晴空指数来对输出进行结构化约束，确保预测严格遵守昼夜周期并防止夜间误差。在苏丹恩图曼（Omdurman）的多年度数据集上进行的验证表明，PISSM 以少于 40,000 个参数实现了卓越的准确性，为实时离网控制建立了一个超轻量级的基准。
