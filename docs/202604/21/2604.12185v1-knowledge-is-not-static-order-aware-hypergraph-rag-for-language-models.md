...
tldr: 针对现有RAG方法将检索证据视为无序集合、忽略交互顺序的问题，本文提出OKH-RAG。该方法将知识表示为具有优先结构的超图，并将检索重新定义为超边上的序列推理。通过学习转移模型自动推断交互顺序，OKH-RAG在热带气旋和港口运营等顺序敏感任务中表现优异，证明了组织结构化序列对有效推理的重要性。
selection_source: fresh_fetch
motivation: 现有的RAG方法假设检索事实是置换不变的，无法处理结果依赖于交互顺序的现实世界推理任务。
method: 提出OKH-RAG，利用带有优先结构的超图表示知识，并通过学习到的转移模型将检索过程建模为超边序列推理。
result: 在热带气旋和港口运营等顺序敏感的问答任务中，OKH-RAG显著优于传统的置换不变基准模型。
conclusion: 有效的推理不仅需要检索相关证据，还必须将其组织成反映底层逻辑的结构化序列。
---
