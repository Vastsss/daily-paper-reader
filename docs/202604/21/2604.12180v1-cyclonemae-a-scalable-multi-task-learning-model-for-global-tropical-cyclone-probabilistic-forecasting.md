...
tldr: 针对热带气旋（TC）预报中数值天气预报计算量大及深度学习模型泛化性差的问题，本文提出CycloneMAE。该模型利用结构感知掩码自编码器从多模态数据中学习可迁移特征，结合离散概率网格化机制，实现了全球范围内的多任务确定性与概率预报。实验证明其在气压、风速及短期路径预报上优于主流数值模型，并具有良好的物理可解释性。
selection_source: fresh_fetch
motivation: 传统数值预报计算成本高且难以利用历史数据，而现有深度学习模型多为确定性预报且缺乏跨变量泛化能力。
method: 提出一种基于结构感知掩码自编码器（MAE）的多任务学习框架，通过预训练与微调范式结合离散概率网格化机制进行多模态预测。
result: 在全球五大洋盆的评估中，该模型在120小时内的气压和风速预报以及24小时内的路径预报表现均优于领先的数值预报系统。
conclusion: CycloneMAE为全球热带气旋提供了一个可扩展、概率性且具有物理可解释性的业务化预报新路径。
---

## 摘要
热带气旋（TCs）是破坏性最强的自然灾害之一，但其预报面临着根本性的权衡：数值天气预报（NWP）模型计算成本极高，且难以利用历史数据；而现有的基于深度学习（DL）的智能模型通常针对特定变量且是确定性的，无法在不同预报变量之间进行泛化。在本文中，我们提出了 CycloneMAE，这是一种可扩展的多任务预报模型，它利用结构感知的热带气旋掩码自编码器从多模态数据中学习可迁移的热带气旋表征。通过将离散概率网格化机制与预训练/微调范式相结合，CycloneMAE 能够同时提供确定性预报和概率分布。在全球五个海域的评估中，CycloneMAE 在长达 120 小时的气压和风力预报以及长达 24 小时的路径预报方面均优于领先的 NWP 系统。通过积分梯度的归因分析揭示了具有物理可解释性的学习动态：短期预报主要依赖于卫星图像中的内部核心对流结构，而长期预报则逐渐将注意力转向外部环境因素。我们的框架为业务化热带气旋预报建立了一条可扩展、概率化且可解释的途径。
