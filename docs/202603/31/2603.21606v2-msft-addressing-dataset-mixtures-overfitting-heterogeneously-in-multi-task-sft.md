...
tldr: 针对多任务监督微调（SFT）中不同数据集学习动态不一致导致的过拟合问题，本文提出了mSFT算法。该算法通过迭代识别并剔除过拟合的子任务，并回退至最优检查点继续训练，有效解决了任务间过拟合异质性。实验证明mSFT在多个基准和模型上均优于基准方法，且能在降低计算开销的同时提升模型性能。
selection_source: fresh_fetch
motivation: 传统多任务SFT对所有数据集采用统一的计算预算，导致学习较快的任务过早过拟合，而学习较慢的任务训练不足。
method: 提出一种迭代式过拟合感知搜索算法，动态识别并排除已过拟合的子数据集，并回退到该任务的最优状态继续训练。
result: 在10个基准测试和6个基础模型上，mSFT一致优于4种基准方法，且对超参数不敏感，具有很强的鲁棒性。
conclusion: mSFT为多任务SFT提供了一种实用的过拟合感知方案，能有效平衡不同任务的训练程度并最大化模型潜力。
---

## 摘要
当前的语言模型训练通常在所有子数据集上使用同质的计算预算进行多任务监督微调（SFT）。这种方法从根本上说是次优的：异质的学习动态导致学习较快的任务过早过拟合，而学习较慢的任务仍处于欠拟合状态。为解决这一问题，我们提出了 mSFT，这是一种针对多任务数据混合的迭代式、过拟合感知的搜索算法。mSFT 在活跃的混合数据上训练模型，识别并排除最早过拟合的子数据集，并在继续训练前回退到该特定的最优检查点。广泛的评估表明，mSFT 在 10 个基准测试和 6 个基座模型上始终优于 4 个基准方法。进一步的分析证实，mSFT 在不同的数据集规模、任务粒度下均能保持稳健的增益，且对其唯一的新超参数（计算预算）不敏感。值得注意的是，在低计算预算下，mSFT 可以在降低训练 FLOPs 的同时提高性能。最终，mSFT 为多任务 SFT 建立了一种实用的过拟合感知算法，能够最大限度地发挥模型在多样化数据混合中的潜力。
