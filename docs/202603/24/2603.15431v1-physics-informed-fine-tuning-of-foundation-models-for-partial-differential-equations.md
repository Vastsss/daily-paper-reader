...
tldr: 针对偏微分方程（PDE）基础模型在下游任务中面临的数据稀缺和分布偏移挑战，本文提出了一种物理信息微调框架。该方法将PDE残差和边界条件等物理约束直接引入微调目标，使模型在无需大量标注数据的情况下也能实现有效适配。实验表明，该框架在未知PDE类别上表现优异，且结合少量数据的混合微调策略能显著提升模型的泛化能力和物理一致性，为科学机器学习提供了高效的适配方案。
selection_source: fresh_fetch
motivation: 现有的PDE基础模型在迁移到新物理系统时，常受限于下游任务标注数据匮乏及分布偏移问题。
method: 提出一种物理信息微调框架，通过在微调目标中加入PDE残差和边界条件等物理约束来优化预训练模型。
result: 该方法在无需PDE解数据的情况下即可达到竞争性精度，且混合微调策略在极少数据下展现出卓越的泛化性能。
conclusion: 物理信息微调为科学机器学习中基础模型的适配提供了一种高效、可解释且具备数据效率的新范式。
---

## 摘要
偏微分方程（PDE）基础模型已作为在多种物理系统上预训练的强大代理模型而出现，但由于任务特定数据有限以及分布偏移，将其适配到新的下游任务仍然具有挑战性。虽然微调在自然语言处理领域已被证明具有变革性，但适配 PDE 基础模型的最佳实践仍有待探索。尽管物理信息训练已成功在广泛的 PDE 问题中训练出精确的求解器，但其在微调基于数据的基础模型方面的潜力尚未得到系统研究。在这项工作中，我们引入了一个物理信息微调框架，通过将物理约束（PDE 残差和边界条件）直接纳入微调目标，来适配预训练的 PDE 基础模型。这使得在数据稀缺的情况下能够进行有效的适配，同时促进物理一致性。我们在由未见过的 PDE 类别组成的下游任务上评估了我们的方法，并将其与数据驱动的微调方法进行了比较。我们的结果表明，物理信息微调在不需要 PDE 解进行训练的情况下，实现了具有竞争力的准确性。此外，当只有极少量训练数据可用时，混合微调策略在分布外场景中表现出卓越的泛化能力。这些发现确立了物理信息微调作为一种可扩展且数据高效的范式，为科学机器学习中适配基础模型提供了一条具有物理可解释性的途径。
