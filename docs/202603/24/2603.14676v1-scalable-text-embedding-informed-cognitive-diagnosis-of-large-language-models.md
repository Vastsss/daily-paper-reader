...
tldr: 针对现有大语言模型评估指标过于粗略、难以揭示底层推理能力的问题，本文提出了一种可扩展的、结合文本嵌入信息的认知诊断模型。该方法通过联合估计LLM的能力掌握情况和题目-属性Q矩阵，解决了高维潜在属性和大规模题库带来的计算挑战。通过引入AI嵌入先验知识，减少了对人工标注的依赖，为LLM提供了细粒度、可解释的能力画像，并在MATH基准测试中验证了其有效性。
...
## 摘要
大语言模型（LLMs）在各类基准测试中取得了卓越的性能，但现有的评估实践主要依赖于粗粒度的汇总指标，这掩盖了底层的推理能力。在这项工作中，我们提出了将心理测量学中的认知诊断模型（CDMs）应用于 LLM 评估的新方法，通过多维离散能力图谱和对 LLM 优缺点的可解释表征，实现细粒度的诊断。首先，为了在基准规模（超过 1000 个题目）上实现基于 CDM 的评估，我们提出了一种可扩展的方法，联合估计 LLM 的掌握程度图谱和题目-属性 Q 矩阵，解决了高维潜在属性（K > 20）、庞大的题目池以及现有基于边缘极大似然估计的高昂计算成本所带来的关键挑战。其次，我们结合了题目层面的文本信息，为 Q 矩阵构建了基于 AI 嵌入信息的先验，在稳定高维估计的同时，减少了对昂贵的人工标注的依赖。我们开发了一种高效的随机逼近算法来联合估计 LLM 掌握程度图谱和 Q 矩阵，从而在数据拟合与文本嵌入启发式先验之间取得平衡。仿真研究证明了准确的参数恢复。在 MATH Level 5 基准测试中的应用展示了我们方法在 LLM 评估中的实际效用，并揭示了关于 LLM 细粒度能力的深刻见解。
