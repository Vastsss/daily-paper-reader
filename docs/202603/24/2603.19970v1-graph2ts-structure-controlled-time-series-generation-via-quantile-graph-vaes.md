...
title_zh: Graph2TS：基于分位数图变分自编码器的结构控制时间序列生成
...
tldr: 针对高波动时间序列生成中全局结构与局部随机性难以平衡的问题，本文提出Graph2TS模型。该方法将时间序列视为结构骨架与随机残差的结合，利用分位数转移图捕捉全局依赖关系，并通过分位数图条件变分自编码器实现从结构图到时间序列的跨模态生成。实验证明该方法在多种数据集上优于扩散模型和GAN，显著提升了生成的保真度与结构一致性。
selection_source: fresh_fetch
motivation: 传统生成模型在处理高波动信号时，往往面临保持全局时间结构与建模局部随机变异之间的矛盾。
method: 提出一种分位数转移图来表征全局结构，并利用分位数图条件变分自编码器实现从结构图到时间序列的跨模态生成。
result: 在太阳黑子、电力负荷、心电图和脑电图等多种数据集上，该模型在分布保真度、时间对齐和代表性方面均优于扩散模型和GAN基准。
conclusion: 结构控制的跨模态生成为时间序列建模提供了新方向，能有效平衡全局组织与样本级随机性。
---

## 摘要
尽管最近的生成模型能够产生具有接近边缘分布的时间序列，但它们在保留全局时间结构与建模随机局部变化之间往往面临着根本性的矛盾，特别是对于具有弱周期性或不规则周期性的高波动信号。在这种情况下，直接的分布匹配可能会放大噪声或抑制有意义的时间模式。在这项工作中，我们提出了一种时间序列生成的“结构-残差”视角，将时间数据视为结构骨架与随机残差动态的结合，从而推动了全局组织与样本级变异性的分离。基于这一见解，我们使用基于分位数的转移图来表示时间序列结构，该图紧凑地捕捉了全局分布和时间依赖性。基于这种表示，我们提出了 Graph2TS，这是一种以分位数图为条件的变分自编码器，可执行从结构图到时间序列的跨模态生成。通过将生成条件设定在结构而非标签或元数据上，该模型在保留全局时间组织的同时，实现了受控的随机变化。在包括太阳黑子、电力负荷、心电图（ECG）和脑电图（EEG）信号在内的多种数据集上的实验表明，与基于扩散模型和 GAN 的基准模型相比，该模型在分布保真度、时间对齐和代表性方面均有提升，突显了结构控制和跨模态生成作为时间序列建模的一个有前景的方向。
