...
tags: ["query:llm-ts"]
score: 8.0
evidence: 非线性时间序列预测与电力负荷预测
tldr: 本研究针对回声状态网络（ESN）中随机初始化的储备池存在冗余节点、导致计算开销大的问题，提出了一种基于图中心性的剪枝方法。该方法将储备池视为加权有向图，利用中心性指标识别并移除结构重要性较低的节点。实验证明，该方法在显著减小储备池规模的同时，能保持甚至提升预测精度，并保留核心动力学特性。
selection_source: fresh_fetch
motivation: 随机初始化的回声状态网络储备池通常包含大量冗余节点，增加了不必要的计算负担。
method: 将储备池建模为加权有向图，并利用图中心性度量来识别和剪除结构上不重要的节点。
result: 在Mackey-Glass和电力负荷预测任务中，该方法显著降低了储备池规模，且维持或提高了预测准确率。
conclusion: 基于中心性的剪枝策略是优化回声状态网络效率的有效手段，能在保证性能的前提下实现模型轻量化。
---
