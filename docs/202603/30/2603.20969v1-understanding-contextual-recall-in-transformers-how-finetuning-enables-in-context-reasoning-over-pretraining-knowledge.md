...
## 摘要
基于 Transformer 的语言模型在上下文学习（ICL）方面表现出色，它们可以根据上下文示例适应新任务，而无需更新参数。在一种被称为“上下文召回”（contextual recall）的特定 ICL 形式中，在开放式文本上预训练的模型利用成对示例，在新的提示格式中召回特定事实。我们研究了上下文召回是否仅通过预训练就能产生，需要什么样的微调，以及哪些机制驱动了必要的表示。为此，我们引入了一个受控的合成框架，其中预训练序列由“主体-语法-属性”元组组成，属性类型与语法统计数据相关联。我们证明，虽然这种预训练成功地产生了事实性知识，但对于上下文召回来说是不够的：当 ICL 提示中移除语法统计数据时，模型无法隐式推断属性类型。然而，我们表明，在与 ICL 评估不同的、需要隐式推断的任务上，使用一部分主体的子集进行微调，会触发所有主体上下文召回能力的出现。这一转变伴随着共享属性类型的低维潜编码（latent encodings）的形成。为了获得机制上的洞察，我们推导出了一个仅包含注意力的 Transformer 结构，该结构复制了从事实召回到上下文召回的转变，并得到了实证验证的支持。
