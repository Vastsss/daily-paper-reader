...
## 摘要
大语言模型（LLMs）在多种自然语言任务中展现出了强大的能力；然而，它们解决带有约束的抽象和优化问题的能力仍鲜有探索。在本文中，我们研究了 LLMs 是否能在最优潮流（OPF）问题的物理和运行约束下进行推理与优化。我们引入了一个具有挑战性的评估设置，该设置要求具备一系列基础技能，如推理、结构化输入处理、算术和约束优化。我们的评估显示，最先进的（SoTA）LLMs 在大多数任务中都失败了，且推理型 LLMs 在最复杂的设置中仍然表现不佳。我们的研究结果突显了 LLMs 在处理约束下结构化推理能力方面的关键差距，本研究为开发能够解决现实世界电网优化问题的更强大的 LLM 助手提供了一个严谨的测试环境。
