...
title_zh: 长航时无人战斗机动力系统：基于一维卷积神经网络的战术级 IPMSM 驱动实时故障分类
...
motivation: 传统故障诊断方法在变转速工况下存在高延迟和泛化能力差的问题，难以满足高可靠性推进系统的实时性需求。
method: 提出一种轻量化的一维卷积神经网络框架，直接处理高频电流和速度信号以实现嵌入式端的实时故障特征提取。
result: 该方法在±2700 RPM范围内实现了0.9834的加权F1分数，性能优于传统的机器学习和长短期记忆网络（LSTM）模型。
conclusion: 该研究为任务关键型电力推进系统提供了一种高效、鲁棒且具备亚毫秒级推理能力的实时故障分类解决方案。
---

## 摘要
针对关键任务应用的高性能动力系统要求前所未有的可靠性和实时故障恢复能力。传统的诊断方法（基于信号的分析和标准机器学习模型）对于定子/转子故障检测至关重要，但在变速条件下存在高延迟和泛化能力差的问题。本文提出了一种用于 HPDM-350 内置式永磁同步电机 (IPMSM) 实时故障分类的一维卷积神经网络 (1-D CNN) 框架。所提出的架构直接从高频电流和转速信号中提取判别性特征，从而在嵌入式控制器上实现亚毫秒级推理。与最先进的长短期记忆网络 (LSTM) 和经典机器学习方法相比，1-D CNN 获得了 0.9834 的优异加权 F1 分数。通过高保真磁域 MATLAB/Simscape 模型验证，该方法在 ±2700 RPM 范围内表现出稳健的性能，为关键任务电力推进系统提供了一种轻量化解决方案。
