...
tldr: 针对Transformer缺乏显式知识存储机制的问题，本文提出“章节混合”（Mixture of Chapters）架构。通过引入可学习的稀疏记忆库并结合受MoE启发的路由机制，将记忆库划分为多个章节进行按需检索。该方法在保持计算效率的同时，将记忆容量扩展至262K个Token，在预训练和指令微调任务中均优于同等算力的基准模型，并显著提升了知识保留和抗遗忘能力。
selection_source: fresh_fetch
motivation: Transformer架构缺乏专门用于存储和组织训练所获知识的显式机制。
method: 引入由潜在Token组成的可学习稀疏记忆库，并利用章节路由机制实现大规模内存的高效检索。
result: 实验表明该模型在多项基准测试中优于同等算力的基准模型，并能有效缓解持续学习中的遗忘问题。
conclusion: 显式关联记忆为模型提供了与隐式参数互补的存储容量，开辟了Transformer扩展的新维度。
---
