...
motivation: 现有的整数值时间序列模型在处理偏态和双峰分布时灵活性不足，且模型解释性有待提高。
method: 提出一种基于负值和非负值组件混合构造的观测驱动型模型，并采用混合泊松拟极大似然估计法进行参数估计。
result: 理论上证明了模型的平稳性、混合性及估计量的渐近性质，并提出了用于残差诊断的Portmanteau检验。
conclusion: 该模型在处理具有复杂分布特征的整数值数据（如金融跳动价格）方面表现出显著的实用性和优越性。
---

## 摘要
本文引入了一类灵活的观测驱动型 $\mathbb{Z}$ 值时间序列模型，该模型由负分量和非负分量的混合构建而成。与基于标准 Skellam 分布或两个整数值变量之差的模型相比，我们的设定提供了更大的灵活性。例如，它能够轻松处理偏态和双峰性。此外，对混合模型中其中一个分量的观测使得解释和统计分析变得更加容易。我们建立了平稳性和混合性（mixing）的条件，并开发了一种具有渐近性质证明的混合泊松准极大似然估计量。提出了一种混成检验（portmanteau test）来诊断残差的序列相关性。通过模拟评估了该方法的有限样本性能，对逐笔报价（tick prices）的实证应用证明了其在实际中的有用性。
