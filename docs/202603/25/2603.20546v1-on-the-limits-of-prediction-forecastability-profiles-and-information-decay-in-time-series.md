...
tags: ["query:llm-ts"]
score: 8.0
evidence: 用于时间序列预测准确性的信息论工具
tldr: 本文利用信息论工具探讨了时间序列预测的根本极限。研究将对数损失分解为不可约的条件熵和模型近似误差，并定义“可预测性”为未来观测值与已知信息间的互信息。通过构建随预测步长变化的可预测性剖面，识别出具有预测价值的“信息视野集”。该框架提供了一种建模前的诊断方法，能够区分数据本身的不可预测性与模型误差，为评估预测可行性和分配建模精力提供了理论依据。
selection_source: fresh_fetch
motivation: 旨在建立一套理论框架以量化预测准确性的上限，并区分数据固有的随机性与模型带来的误差。
method: 采用信息论方法将预测损失分解，通过互信息定义可预测性，并分析其在不同预测步长下的演化特征。
result: 提出了可预测性剖面和信息视野集的概念，揭示了预测信息在不同时间跨度上的分布规律，包括非单调的季节性特征。
conclusion: 该研究为预测任务提供了一种原则性的预建模诊断工具，帮助识别可预测的时间范围，从而优化建模策略。
---

## 摘要
预测准确性受限于关于未来的可用信息。本文利用信息论工具对这一陈述进行了精确化。在对数损失下，任何概率预测的期望性能都可以分解为两个部分：不可约成分和近似成分。不可约项是给定可用信息时未来的条件熵，而近似项是真实条件分布与预测方法之间的散度。这一条件熵极限与无条件基准之间的差距，正是未来观测值与声明的信息集之间的互信息。由此引出了可预测性的定义，即期望对数损失中可实现的最大降幅。在不同预测步长上进行评估，可预测性形成了一个剖面（profile），描述了预测信息随提前期的变化情况。该剖面反映了过程的依赖结构，且不必是单调的：预测信息可能集中在特定的滞后阶数（包括季节性步长），即使中间步长几乎不包含有用信号。基于该剖面，本文定义了信息步长集：即可预测性超过实际阈值的步长集合。在不属于该集合的步长上，无论使用何种预测方法，相对于无条件基准的可实现增益必然很小。因此，该框架将可学习的内容与不可学习的内容分开，并区分了由数据强加的限制与由建模引入的误差。其结果是一种建模前诊断工具，可在选择任何模型之前识别出哪些地方可行有效的预测，为在不同预测步长上分配建模工作量提供了原则性依据。
