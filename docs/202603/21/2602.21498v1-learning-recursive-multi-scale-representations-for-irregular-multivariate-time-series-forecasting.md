...
motivation: 现有的多尺度IMTS预测方法多采用重采样技术，这会改变原始时间戳并破坏其中蕴含的关键采样模式信息。
method: 提出一种递归多尺度建模方法ReIMTS，通过保持时间戳不变并递归划分子样本，结合不规则感知融合机制捕捉全局到局部的依赖。
result: "在多个真实世界数据集的预测任务中，ReIMTS相比现有模型实现了平均27.1%的性能提升。"
conclusion: ReIMTS通过递归多尺度表示学习有效保留了不规则采样特征，为不规则多元时间序列预测提供了一种高效的建模方案。
---
