...
tldr: "本研究针对美国电网预测任务，系统性地评估了状态空间模型（PowerMamba, S-Mamba）、Transformer（iTransformer, PatchTST）及LSTM的性能。通过在六个电网的负荷、风电、光伏及电价数据上进行多维度实验，研究揭示了模型表现与数据特性之间的强耦合关系：PatchTST在处理强周期性信号时领先，而iTransformer在融合天气协变量方面效率更高，SSM则更擅长应对风电和电价的剧烈波动。"
selection_source: carryover_cache
motivation: 旨在解决电网运营商在面对不同数据环境时，难以从众多深度学习架构中选择最优预测模型的问题。
method: 通过统一的时间处理和天气变量集成模块，在六个美国电网的多种能源数据上对五种主流神经架构进行了公平的基准测试。
result: 实验表明没有单一模型在所有场景下胜出，PatchTST擅长周期性信号，iTransformer在处理多变量天气信息时最有效，而SSM在处理混沌波动数据时表现更佳。
conclusion: 研究为电网运营商提供了实用指南，强调应根据具体的预测任务特性和可用数据类型来选择最合适的模型架构。
---
