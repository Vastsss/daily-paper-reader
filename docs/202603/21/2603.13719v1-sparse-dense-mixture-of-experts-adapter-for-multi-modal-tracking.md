...
## 摘要
参数高效微调（PEFT）技术（如提示和适配器）在多模态跟踪中得到了广泛应用，因为它们缓解了全模型微调带来的时间效率低、资源消耗高、参数存储负担重以及灾难性遗忘等问题。然而，由于跨模态异质性，大多数现有的基于 PEFT 的方法难以在具有共享参数的统一框架内有效表示多模态特征。为了解决这一问题，我们提出了一种新颖的稀疏-稠密混合专家适配器（SDMoEA）框架，用于统一模型结构下基于 PEFT 的多模态跟踪。具体而言，我们设计了一个 SDMoE 模块作为多模态适配器，以高效建模模态特定信息和共享信息。SDMoE 由稀疏 MoE 和稠密共享 MoE 组成：前者捕获模态特定信息，而后者建模共享的跨模态信息。此外，为了克服现有跟踪方法在多级多模态融合过程中建模高阶相关性的局限性，我们引入了基于 Gram 矩阵的语义对齐超图融合（GSAHF）模块。该模块首先利用 Gram 矩阵进行跨模态语义对齐，确保构建的超图能够准确反映模态间的语义相似性和高阶依赖关系。随后，对齐后的特征被集成到超图结构中，利用其建模高阶关系的能力，实现多级多模态信息的深度融合。大量实验表明，与其它 PEFT 方法相比，所提方法在 LasHeR、RGBT234、VTUAV、VisEvent、COESOT、DepthTrack 和 VOT-RGBD2022 等多个多模态跟踪基准测试中均取得了优越的性能。
