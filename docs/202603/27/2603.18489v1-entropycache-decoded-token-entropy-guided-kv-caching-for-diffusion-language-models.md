...
tldr: 针对扩散语言模型（dLLM）因双向注意力导致无法实现无损KV缓存且推理开销大的问题，本文提出EntropyCache。这是一种无需训练的KV缓存方法，通过计算新解码Token分布的最大熵作为信号，决定何时重新计算缓存。该方法利用熵与缓存漂移的相关性，仅需常数级计算开销，在保持竞争力的准确率下，显著提升了LLaDA和Dream等模型的推理速度。
selection_source: fresh_fetch
motivation: 扩散语言模型的双向注意力机制使得传统KV缓存失效，而现有近似缓存方法的决策开销随上下文长度或模型深度增加。
method: 提出以解码Token的最大熵作为缓存过时程度的低成本代理指标，并结合对最近k个Token的重新计算策略。
result: "在LLaDA-8B和Dream-7B模型上实现了15.2倍至26.4倍的推理加速，且决策开销仅占推理时间的0.5%。"
conclusion: EntropyCache通过极低的计算成本实现了高效的KV缓存管理，为扩散语言模型的大规模推理提供了实用且高性能的解决方案。
---

## 摘要
基于扩散的大语言模型 (dLLMs) 依赖于双向注意力机制，这阻碍了无损 KV 缓存的实现，并要求在每个去噪步骤中进行完整的正向传递。现有的近似 KV 缓存方法通过选择性地更新缓存状态来降低这一成本，但其决策开销随上下文长度或模型深度而增加。我们提出了 EntropyCache，这是一种无需训练的 KV 缓存方法，它利用新解码 Token 分布的最大熵作为恒定成本信号，用于决定何时重新计算。我们的设计基于两个经验观察：(1) 解码 Token 的熵与 KV 缓存漂移相关，为缓存陈旧度提供了一个廉价的代理指标；(2) 解码 Token 的特征波动在取消掩码后会持续多个步骤，这促使我们重新计算最近解码的 $k$ 个 Token。跳过或重新计算的决策每步仅需 $O(V)$ 的计算量，与上下文长度和模型规模无关。在 LLaDA-8B-Instruct 和 Dream-7B-Instruct 上的实验表明，EntropyCache 在标准基准测试中实现了 15.2x-26.4x 的加速，在思维链 (CoT) 基准测试中实现了 22.4x-24.1x 的加速，同时具有竞争力的准确率，且决策开销仅占推理时间的 0.5%。代码可在 https://github.com/mscheong01/EntropyCache 获取。
